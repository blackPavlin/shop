...
func (pr *Product) NamedCarts(name string) ([]*Cart, error) {
	if pr.Edges.namedCarts == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := pr.Edges.namedCarts[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (pr *Product) appendNamedCarts(name string, edges ...*Cart) {
	if pr.Edges.namedCarts == nil {
		pr.Edges.namedCarts = make(map[string][]*Cart)
	}
	if len(edges) == 0 {
		pr.Edges.namedCarts[name] = []*Cart{}
	} else {
		pr.Edges.namedCarts[name] = append(pr.Edges.namedCarts[name], edges...)
	}
}
